...
import { Check, ChevronRight, ChevronLeft, RefreshCw, Copy, CheckCircle2, Download } from 'lucide-react';
...
const BrokerSetupCalculator = () => {
  ...
  const handleDownloadPDF = () => {
    ...
    // Title
    doc.setFontSize(18);
    doc.text("Brokerage Quote Summary", 14, 22);
    doc.setFontSize(11);
    doc.setTextColor(100);
    doc.text("Generated by Kasper Capital Brokerage Setup Calculator", 14, 30);
    
    let currentY = 40;
    const active = getActiveOptionsList();
    
    // Loop through steps to create tables
    steps.forEach(step => {
      const stepOptions = active.filter(opt => opt.stepId === step.id);
      if (stepOptions.length > 0) {
        
        const tableData = stepOptions.map(opt => {
          const costs = [];
          if (opt.oneTime) costs.push(`One-Time: $${opt.oneTime.toLocaleString()}`);
          if (opt.totalCost) costs.push(`One-Time: $${opt.totalCost.toLocaleString()}`);
          if (opt.deposit) costs.push(`Deposit: $${opt.deposit.toLocaleString()}`);
          if (opt.monthly) costs.push(`Monthly: $${opt.monthly.toLocaleString()}`);
          if (opt.renewal) costs.push(`Renewal: $${opt.renewal.toLocaleString()}`);
          return [opt.name, costs.join(' | ')];
        });

        autoTable(doc, {
          startY: currentY,
          head: [[step.title, 'Costs']],
          body: tableData,
          theme: 'grid',
          headStyles: { fillColor: [0, 204, 255], textColor: [0, 0, 0] },
          margin: { left: 14 }
        });
        
        currentY = (doc as any).lastAutoTable.finalY + 10;
        
        // Add a new page if currentY is too low
        if (currentY > 270) {
          doc.addPage();
          currentY = 20;
        }
    ...
    const t = calculateTotals();
    const breakdownData = [
      ['One-Time', `$${t.oneTime.toLocaleString()}`],
      ['Monthly', `$${t.monthly.toLocaleString()}`],
      ['Annualized Monthly', `$${t.annualizedMonthly.toLocaleString()}`],
      ['Compliance Renewal', `$${t.renewal.toLocaleString()}`],
      ['Year 1 Total', `$${t.year1Total.toLocaleString()}`]
    ];
                  ...
                  <div>
                    ...
                    <div className="flex">
                      <select 
                        ...
                        className="bg-[#1A1A1A] text-gray-400 rounded-l-lg px-2 py-3 outline-none border border-r-0 border-[#262626] focus:border-accent-cyan transition-colors cursor-pointer"
                      >
                        {COUNTRY_CODES.map(c => (
                      ...
                      <input 
                        ...
                        className="flex-1 w-full bg-[#1A1A1A] text-white rounded-r-lg px-4 py-3 outline-none border border-[#262626] focus:border-accent-cyan transition-colors" 
                   ...
                   (isRegulated ? complianceOptions.regulated : complianceOptions.nonRegulated).map(opt => {
                     ...
                     return (
                       <motion.div 
                         ...
                         className={`relative cursor-pointer p-6 rounded-2xl transition-all duration-300 ${isSelected ? 'border-[1.5px] border-accent-cyan bg-black shadow-[0_0_20px_rgba(0,229,255,0.2)]' : 'border border-[#262626] bg-[#0A0A0A] hover:border-white/30 hover:shadow-xl'}`}
                       >
                  ...
                  (currentStep.options as any[])?.map((opt: any) => {
                    ...
                    return (
                      <motion.div 
                        ...
                        whileHover={{ scale: 1.02, y: -5 }}
                        onClick={() => toggleSelection(currentStep.id, opt.id, currentStep.type)}
                        className={`relative cursor-pointer p-6 rounded-2xl transition-all duration-300 flex flex-col justify-between ${isSelected ? 'border-[1.5px] border-accent-cyan bg-black shadow-[0_0_20px_rgba(0,229,255,0.2)]' : 'border border-[#262626] bg-[#0A0A0A] hover:border-white/30 hover:shadow-xl'}`}
                      >
                        {isSelected && <div className="absolute top-4 right-4 text-accent-cyan"><Check size={20} /></div>}
                        <div>
                    ...
                    <PieChart>
                      <Pie
                        data={[
                          ...
                          { name: 'Ann. Monthly', value: totals.annualizedMonthly },
                          { name: 'Renewal', value: totals.renewal }
                        ].filter(d => d.value > 0)}
                        cx="50%"
                        cy="50%"
                        innerRadius={50}
                      ...
                      >
                        {
                          [
                            ...
                            <Cell key={`cell-${index}`} fill={['#00E5FF', '#00E676', '#FF6B4A'][index % 3]} />
                          ))
                        }
                      </Pie>
                      <RechartsTooltip 
